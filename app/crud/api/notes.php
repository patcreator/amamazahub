...
try {
    switch($action) {
        ...
        $status = trim($_POST['status'] ?? '');
            ...
            isNot(['id', 'title', 'content', 'updated_at', 'created_at', 'status']);
            // Check if column exists
            
            $stmt = $pdo->prepare("INSERT INTO notes (id, title, content, updated_at, created_at, status) VALUES (?, ?, ?, ?, ?, ?)");
            if($stmt->execute([$id, $title, $content, $updated_at, $created_at, $status])) {
        ...
        $status = trim($_POST['status'] ?? '');
            ...
            $stmt = $pdo->prepare("UPDATE notes SET id=?, title=?, content=?, updated_at=?, created_at=?, status=? WHERE id=?");
            if($stmt->execute([$id, $title, $content, $updated_at, $created_at, $status,  $id])) {
        ...
        case 'bulk_edit':
            foreach($_POST['ids'] as $id){
                $stmt = $pdo->prepare("UPDATE notes SET id=?, title=?, content=?, updated_at=?, created_at=?, status=? WHERE id=?");
                $stmt->execute([$id, $title, $content, $updated_at, $created_at, $status,  $id]);
            }
        ...
        case 'copy':
            ...
            $stmt->execute([$id]);
            $notes = $stmt->fetch(PDO::FETCH_ASSOC);
            if(!$notes) throw new Exception("Data not found");

            $stmt = $pdo->prepare("INSERT INTO notes (id, title, content, updated_at, created_at, status) VALUES (?, ?, ?, ?, ?, ?)");
            $stmt->execute([$notes['email'], $notes['email'], $notes['email'], $notes['email'], $notes['email'], $notes['email']]);
        ...
        $status = trim($_POST['status'] ?? '');
                ...
                $stmt = $pdo->prepare("INSERT INTO notes (id, title, content, updated_at, created_at, status) VALUES (?, ?, ?, ?, ?, ?)");
                $stmt->execute([$id, $title, $content, $updated_at, $created_at, $status]);
            }
        ...
        case 'copy_many':
            ...
            $notes = $stmt->fetchAll(PDO::FETCH_ASSOC);
            foreach($notes as $u){
                $stmt2 = $pdo->prepare("INSERT INTO notes (id, title, content, updated_at, created_at, status) VALUES (?, ?, ?, ?, ?, ?)");
        ...
        $status = trim($_POST['status'] ?? '');
            ...
            $stmt = $pdo->prepare("UPDATE notes SET id=?, title=?, content=?, updated_at=?, created_at=?, status=? WHERE id IN (".implode(',', array_fill(0,count($ids),'?')).")");
            $stmt->execute(array_merge([$id, $title, $content, $updated_at, $created_at, $status, ], $ids));
        ...
        case 'inline_update':
            ...
            $allowed_fields = ['id', 'title', 'content', 'updated_at', 'created_at', 'status'];
