...
try {
    switch($action) {
        ...
        $status = trim($_POST['status'] ?? '');
            ...
            $stmt = $pdo->prepare("UPDATE contacts SET id=?, name=?, phone=?, email=?, created_at=?, updated_at=?, status=? WHERE id=?");
        ...
        case 'copy':
            ...
            $stmt->execute([$id]);
            $contacts = $stmt->fetch(PDO::FETCH_ASSOC);
            if(!$contacts) throw new Exception("Data not found");

            $stmt = $pdo->prepare("INSERT INTO contacts (id, name, phone, email, created_at, updated_at, status) VALUES (?, ?, ?, ?, ?, ?, ?)");
            $stmt->execute([$contacts['email'], $contacts['email'], $contacts['email'], $contacts['email'], $contacts['email'], $contacts['email'], $contacts['email']]);
