...
try {
    switch($action) {
        ...
        $status = trim($_POST['status'] ?? '');
            ...
            $stmt = $pdo->prepare("UPDATE todos SET id=?, task=?, completed=?, created_at=?, updated_at=?, status=? WHERE id=?");
        ...
        case 'copy':
            ...
            $stmt->execute([$id]);
            $todos = $stmt->fetch(PDO::FETCH_ASSOC);
            if(!$todos) throw new Exception("Data not found");

            $stmt = $pdo->prepare("INSERT INTO todos (id, task, completed, created_at, updated_at, status) VALUES (?, ?, ?, ?, ?, ?)");
            $stmt->execute([$todos['email'], $todos['email'], $todos['email'], $todos['email'], $todos['email'], $todos['email']]);
