...
try {
    switch($action) {
        ...
        $status = trim($_POST['status'] ?? '');
            ...
            $stmt = $pdo->prepare("UPDATE help_topics SET id=?, title=?, content=?, created_at=?, updated_at=?, status=? WHERE id=?");
        ...
        case 'copy':
            ...
            $stmt->execute([$id]);
            $help_topics = $stmt->fetch(PDO::FETCH_ASSOC);
            if(!$help_topics) throw new Exception("Data not found");

            $stmt = $pdo->prepare("INSERT INTO help_topics (id, title, content, created_at, updated_at, status) VALUES (?, ?, ?, ?, ?, ?)");
            $stmt->execute([$help_topics['email'], $help_topics['email'], $help_topics['email'], $help_topics['email'], $help_topics['email'], $help_topics['email']]);
