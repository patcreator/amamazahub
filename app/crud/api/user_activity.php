...
try {
    switch($action) {
        ...
        $status = trim($_POST['status'] ?? '');
            ...
            $stmt = $pdo->prepare("UPDATE user_activity SET id=?, user_id=?, activity_type=?, description=?, ip_address=?, created_at=?, updated_at=?, status=? WHERE id=?");
        ...
        case 'copy':
            ...
            $stmt->execute([$id]);
            $user_activity = $stmt->fetch(PDO::FETCH_ASSOC);
            if(!$user_activity) throw new Exception("Data not found");

            $stmt = $pdo->prepare("INSERT INTO user_activity (id, user_id, activity_type, description, ip_address, created_at, updated_at, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
            $stmt->execute([$user_activity['email'], $user_activity['email'], $user_activity['email'], $user_activity['email'], $user_activity['email'], $user_activity['email'], $user_activity['email'], $user_activity['email']]);
