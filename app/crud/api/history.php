...
try {
    switch($action) {
        ...
        $status = trim($_POST['status'] ?? '');
            ...
            $stmt = $pdo->prepare("UPDATE history SET id=?, user_id=?, action=?, entity_type=?, entity_id=?, old_value=?, new_value=?, ip_address=?, user_agent=?, created_at=?, updated_at=?, status=? WHERE id=?");
        ...
        case 'copy':
            ...
            $stmt->execute([$id]);
            $history = $stmt->fetch(PDO::FETCH_ASSOC);
            if(!$history) throw new Exception("Data not found");

            $stmt = $pdo->prepare("INSERT INTO history (id, user_id, action, entity_type, entity_id, old_value, new_value, ip_address, user_agent, created_at, updated_at, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
            $stmt->execute([$history['email'], $history['email'], $history['email'], $history['email'], $history['email'], $history['email'], $history['email'], $history['email'], $history['email'], $history['email'], $history['email'], $history['email']]);
