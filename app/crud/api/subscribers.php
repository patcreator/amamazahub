...
try {
    switch($action) {
        ...
        $updated_at = trim($_POST['updated_at'] ?? '');
            ...
            isNot(['id', 'email', 'status', 'created_at', 'updated_at']);
            // Check if column exists
            
            $stmt = $pdo->prepare("INSERT INTO subscribers (id, email, status, created_at, updated_at) VALUES (?, ?, ?, ?, ?)");
            if($stmt->execute([$id, $email, $status, $created_at, $updated_at])) {
        ...
        $updated_at = trim($_POST['updated_at'] ?? '');
            ...
            $stmt = $pdo->prepare("UPDATE subscribers SET id=?, email=?, status=?, created_at=?, updated_at=? WHERE id=?");
            if($stmt->execute([$id, $email, $status, $created_at, $updated_at,  $id])) {
        ...
        case 'bulk_edit':
            foreach($_POST['ids'] as $id){
                $stmt = $pdo->prepare("UPDATE subscribers SET id=?, email=?, status=?, created_at=?, updated_at=? WHERE id=?");
                $stmt->execute([$id, $email, $status, $created_at, $updated_at,  $id]);
            }
        ...
        case 'copy':
            ...
            $stmt->execute([$id]);
            $subscribers = $stmt->fetch(PDO::FETCH_ASSOC);
            if(!$subscribers) throw new Exception("Data not found");

            $stmt = $pdo->prepare("INSERT INTO subscribers (id, email, status, created_at, updated_at) VALUES (?, ?, ?, ?, ?)");
            $stmt->execute([$subscribers['email'], $subscribers['email'], $subscribers['email'], $subscribers['email'], $subscribers['email']]);
        ...
        $updated_at = trim($_POST['updated_at'] ?? '');
                ...
                $stmt = $pdo->prepare("INSERT INTO subscribers (id, email, status, created_at, updated_at) VALUES (?, ?, ?, ?, ?)");
                $stmt->execute([$id, $email, $status, $created_at, $updated_at]);
            }
        ...
        case 'copy_many':
            ...
            foreach($subscribers as $u){
                $stmt2 = $pdo->prepare("INSERT INTO subscribers (id, email, status, created_at, updated_at) VALUES (?, ?, ?, ?, ?)");
        ...
        $updated_at = trim($_POST['updated_at'] ?? '');
            ...
            $stmt = $pdo->prepare("UPDATE subscribers SET id=?, email=?, status=?, created_at=?, updated_at=? WHERE id IN (".implode(',', array_fill(0,count($ids),'?')).")");
            $stmt->execute(array_merge([$id, $email, $status, $created_at, $updated_at, ], $ids));
        ...
        case 'inline_update':
            ...
            $allowed_fields = ['id', 'email', 'status', 'created_at', 'updated_at'];
