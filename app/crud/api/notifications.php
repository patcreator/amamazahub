...
try {
    switch($action) {
        ...
        $status = trim($_POST['status'] ?? '');
            ...
            $stmt = $pdo->prepare("UPDATE notifications SET id=?, user_id=?, sender_id=?, title=?, message=?, type=?, link=?, is_read=?, created_at=?, updated_at=?, status=? WHERE id=?");
        ...
        case 'copy':
            ...
            $stmt->execute([$id]);
            $notifications = $stmt->fetch(PDO::FETCH_ASSOC);
            if(!$notifications) throw new Exception("Data not found");

            $stmt = $pdo->prepare("INSERT INTO notifications (id, user_id, sender_id, title, message, type, link, is_read, created_at, updated_at, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
            $stmt->execute([$notifications['email'], $notifications['email'], $notifications['email'], $notifications['email'], $notifications['email'], $notifications['email'], $notifications['email'], $notifications['email'], $notifications['email'], $notifications['email'], $notifications['email']]);
